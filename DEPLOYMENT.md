...
### 2. .env Dosyasını Oluştur (DÜZELTİLDİ)
**ÖNEMLİ DÜZELTME:** Docker içinde `127.0.0.1` SADECE kendi kutusunu görür. Diğer servislere ulaşmak için **servis isimlerini** (mysql, redis) kullanmalısınız.
`nano .env` yaparak içeriği şununla değiştirin:

```properties
# --- Veritabanı (Docker İçi Ağ) ---
DB_HOST=mysql
DB_PORT=3306
...
# --- Redis (Docker İçi Ağ) ---
# Burasi 127.0.0.1 OLAMAZ, 'redis' olmali
REDIS_HOST=redis
REDIS_PORT=6379
...
Yapılan IP değişikliklerinin algılanması için konteynerleri kapatıp açmak gerekebilir:
```bash
docker-compose down
